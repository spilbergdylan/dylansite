...
const CelestialBody = ({ name, type, position, size, color, isActive, initialRotation = 0, content, textures, panelConfig }) => {
  ...
  // Function to create content on planet surface
  const createContent = () => {
    if (!content || !isActive) return null;

    const lines = content.split('\n');
    
    // Calculate angles for text wrapping
    const verticalSpread = Math.PI / 6;
    const verticalStart = verticalSpread / 2;
    const charSpacing = Math.PI / 50; // Reduced character spacing for tighter letters
    const wordSpacing = charSpacing * .5; // 1.5x character spacing for words

    return (
      <group rotation={[0, Math.PI, 0]}>
        {lines.map((line, lineIndex) => {
          const verticalAngle = verticalStart - (lineIndex * (verticalSpread / Math.max(lines.length - 1, 1)));
          const chars = line.trim().split('');
          
          // Calculate total line width including word spacing
          const totalSpacing = chars.reduce((total, char, i) => 
            total + (char === ' ' ? wordSpacing : charSpacing), 0);
          const lineStart = -totalSpacing / 2;

          // Keep track of cumulative spacing
          let currentOffset = 0;

          return (
            <group key={lineIndex}>
              {chars.map((char, charIndex) => {
                // Add extra spacing after spaces
                const spacing = char === ' ' ? wordSpacing : charSpacing;
                const horizontalAngle = lineStart + currentOffset;
                ...
                const x = radius * Math.cos(verticalAngle) * Math.sin(horizontalAngle);
                const y = radius * Math.sin(verticalAngle);
                const z = radius * Math.cos(verticalAngle) * Math.cos(horizontalAngle);
                ...
                const rotationMatrix = new THREE.Matrix4().makeBasis(right, correctedUp, normal);
